...
export const getPlaylistsSummary = cache(async (): Promise<PlaylistsSummary | null> => {
  if (process.env.NODE_ENV !== 'production') {
    return randomPlaylists;
  }

  const kv = await getKvNamespace();
  const playlistSummary = await kv.get<PlaylistsSummary>(PLAYLIST_KV_CACHE.PLAYLIST_SUMMARY, { type: 'json' });
...
export const getPlaylist = cache(async (playlistSlug: string): Promise<Playlist | null> => {
  console.log('💕');
  ...
  const playlist = await client.fetch<Playlist | null>(SINGLE_PLAYLIST_QUERY, {
    slug: playlistSlug,
  });
...
export const getLikedPlaylist = cache(async (): Promise<Playlist | null> => {
